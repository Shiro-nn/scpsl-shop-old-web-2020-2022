		...
		if(clan.users.filter(x => x.user == userData.id && x.access >= 2).length > 0){
			async function CreateInv() {
				...
				if(__ == null || __ == undefined){
					let invData = new invsData({code:_code, clan:clan.tag, expires:(Date.now() + 1000 * 60 * 60 * 24), by:userData.id});
					await invData.save();
...
router.post('/api/balance', vhost(host, async(req, res) => {
	if (req.bs.data.user == null){
		...
		const _order = guid();
		const sign = crypto.createHash('sha256').update(`1505:${req.body.sum}:xxx:${_order}`).digest('hex');
        ...
        payData.paymentid++;
		await payData.save();
		res.status(200).send(encodeURI(`https://fowpay.com/pay?shop=1505&order=${_order}&amount=${req.body.sum}&sign=${sign}&desc=Пополнение баланса №${payData.paymentid}`));
	};
}));
